...
def apply_grouped_rotary_pos_emb_q(q, cos, sin, position_ids, g_size_1=8, g_size_2=1024):
    # The first two dimensions of cos and sin are always 1, so we can `squeeze` them.
...
def apply_grouped_rotary_pos_emb_k(k, cos, sin, position_ids, g_size_1=8, g_size_2=1024):
    # The first two dimensions of cos and sin are always 1, so we can `squeeze` them.
...
def self_extend_forward(
    ...
    **kwargs,
    ...
    ################################### above is same #######################################
    #neighbor_query_states = apply_rotary_pos_emb_single(query_states, cos, sin, position_ids) # normal attention 
    neighbor_query_states = apply_rotary_pos_emb_single(query_states, cos, sin, position_ids) # normal attention 
    neighbor_key_states = key_states
    ...
    #group_query_states= apply_grouped_rotary_pos_emb_q(query_states, cos, sin, position_ids, g_size_1=group_size_1, g_size_2=_re_group_size_2) # grouped attention
    group_query_states = apply_grouped_rotary_pos_emb_q(query_states, cos, sin, position_ids, g_size_1=group_size_1, g_size_2=_re_group_size_2) # grouped attention
    group_key_states = key_states
    ...
    past_key_value = (neighbor_key_states, group_key_states, value_states) if use_cache else None


    ### Shift Pos: key pos is the pos in cache

    key_position_ids = torch.arange(kv_seq_len, device=position_ids.device).unsqueeze(0)
    group_key_states= apply_grouped_rotary_pos_emb_k(group_key_states, cos, sin, key_position_ids, g_size_1=group_size_1, g_size_2=_re_group_size_2)
    neighbor_key_states = apply_rotary_pos_emb_single(neighbor_key_states, cos, sin, key_position_ids)
